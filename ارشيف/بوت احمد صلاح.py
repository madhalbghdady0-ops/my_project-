...
def init_db():
    ...
    # جدول أنظمة المشتركين: (user_id, day, content)
...
async def help_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        "🛠 **لوحة تحكم البوت (الأوامر التفاعلية):**\n\n"
...
# --- محادثة إضافة/تعديل النظام (تفاعلية بالكامل) ---
...
async def plan_get_uid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("الخطوة 2/3: تمام! الآن أرسل **اليوم** (مثلاً: `الأحد`، `الإثنين`، `السبت`):")
    return PLAN_DAY
...
async def plan_finish(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # محاولة إرسال النظام فوراً للمشترك كإشعار
    try:
...
async def remove_plan_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 2:
        await update.message.reply_text("⚠️ الاستخدام الصحيح:\n`/removeplan [ID] [اليوم]`\nمثال: `/removeplan 12345678 الأحد`", parse_mode="Markdown")
        return
...
async def clear_plan_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id): return
    if not context.args:
        await update.message.reply_text("⚠️ الاستخدام الصحيح:\n`/clearplan [ID]`\nلحذف كل أنظمة المشترك تماماً.", parse_mode="Markdown")
        return
    uid = context.args[0]
    conn = sqlite3.connect(DB_FILE)
    ...
    conn.commit()
    conn.close()
    await update.message.reply_text(f"🗑 تم مسح كافة الأنظمة والجدداول الخاصة بالمشترك `{uid}`.", parse_mode="Markdown")

# --- محادثات لوحة التحكم الأخرى ---
...
# --- واجهة المشترك الأساسية ---
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    # تسجيل المستخدم في القاعدة أوتوماتيك
    conn = sqlite3.connect(DB_FILE)
    conn.execute("INSERT OR IGNORE INTO users (user_id, name) VALUES (?, ?)", (str(user.id), user.first_name))
    conn.commit()
    conn.close()

    keyboard = [
        [InlineKeyboardButton("💳 طلب اشتراك VIP", callback_data="buy")],
        [InlineKeyboardButton("🎟 إدخال كود خصم", callback_data="promo_input")],
    ...
    await update.message.reply_text(f"أهلاً بك يا {user.first_name} في البوت الرياضي 🚀\nاختر من الأزرار أدناه:", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "my_plan":
        ...
        # جلب نظام اليوم (مثلاً حسب اليوم الحالي أو عرض كل الأيام)
        conn = sqlite3.connect(DB_FILE)
        c = conn.cursor()
        c.execute("SELECT day, content FROM user_plans WHERE user_id = ?", (user_id,))
        plans = c.fetchall()
        conn.close()
...
def main():
    ...
    print("🚀 البوت بنظام الأكل والتمارين اليومية يعمل بكفاءة تامة!")
    app.run_polling()
