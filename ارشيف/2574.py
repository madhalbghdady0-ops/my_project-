...
def init_db():
    ...
    # جدول للأزرار المخصصة التي يضيفها الأدمن
...
def get_contact_link():
    ...
    return "https://t.me/your_username" # الافتراضي

# --- محادثة تعديل تواصل الكوتش ---
async def set_contact_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text("🔗 أرسل الآن **يوزر أو رابط تواصل الكوتش الجديد** (مثلاً: `@username` أو الرابط كامل):", parse_mode="Markdown")
...
# --- محادثة إضافة زرار مخصص جديد برد مباشر ---
...
async def add_btn_finish(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ **تم إضافة الزرار بنجاح!**\n📌 اسم الزرار: `{btn_name}`\n هيظهر للناس في القائمة الرئيسية فوراً.", parse_mode="Markdown")
    return ConversationHandler.END

# --- محادثات الإدارة السابقة (أكواد الخصم، الأنظمة، الرسائل، الأدمنز) ---
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # جلب الأزرار المخصصة التي أضافها الأدمن من قاعدة البيانات
    c = conn.cursor()
    ...
    if is_owner(user_id):
        ...
        # إضافة الأزرار المخصصة التي أنشأها الأدمن للزبائن في القائمة الرئيسية
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "my_plan":
        ...
        # التعامل مع الضغط على الأزرار المخصصة التي أضافها الأدمن وعرض الرد الخاص بها
        conn = sqlite3.connect(DB_FILE)
        c = conn.cursor()
...
def main():
    ...
    print("🚀 البوت شغال بالكامل بنظام الأزرار الديناميكية، روابط التواصل المتغيرة، وإدارة الأدمنز والمالك!")
    app.run_polling()
