...
EXPORT_TARGET, EXPORT_TYPE, EXPORT_INCLUDE_NOTES = range(30, 33)
...
# --- إحصائيات المشتركين والنظام الشاملة ---
async def admin_stats_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for idx, (uid, name, end_date_str) in enumerate(all_users, 1):
        ...
        if end_date_str:
            try:
                sub_end_date = datetime.strptime(end_date_str, "%Y-%m-%d")
                delta_days = (sub_end_date - today).days
                ...
                if sub_end_date >= today:
                    ...
                    if days_left <= 3:
                        warning_status = " 🚨 (قرب يخلص جداً!)"
                    elif days_left <= 7:
                        warning_status = " ⚠️ (قرب يخلص)"
                else:
                    expired_count += 1
            except:
                expired_count += 1
        ...
        users_details_text += (
            f"👤 **{idx}. العضو:** `{uid}` ({name or 'متدرب'})\n"
            f"• الحالة: {status_label}{warning_status}\n"
            f"• تاريخ الانتهاء: `{end_date_str or 'غير محدد'}`\n"
            f"• الباقي: **{days_left} يوم**\n"
            f"------------------\n"
        )

    stats_report = (
        f"📊 **لوحة إحصائيات المشتركين والنظام الشاملة:**\n\n"
        f"👥 إجمالي المسجلين: **{total_users}**\n"
        f"🟢 الاشتراكات السارية: **{active_count}**\n"
        f"🔴 الاشتراكات المنتهية: **{expired_count}**\n"
        f"🎟 أكواد الخصم النشطة: **{total_promos}** (إجمالي الاستخدام: {total_uses})\n\n"
        f"📋 **تفاصيل المشتركين:**\n"
        f"{users_details_text if users_details_text else 'لا توجد بيانات مسجلة حالياً.'}"
    )

    keyboard = [
        [InlineKeyboardButton("🔄 تحديث الإحصائيات", callback_data="btn_admin_stats")],
        [InlineKeyboardButton("🔙 رجوع للقائمة الرئيسية", callback_data="back_home")]
    ]
    if len(stats_report) > 4000:
        await query.message.reply_text(stats_report[:4000], parse_mode="Markdown")
    else:
        await query.edit_message_text(stats_report, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")

# --- تصدير تقارير البيانات (Excel) ---
async def export_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text("📤 **نظام تصدير وتقارير البيانات:**\n\nاختر نطاق التصدير المطلوبة:", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
...
async def export_get_target(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice == "exp_single":
        ...
        keyboard = [
            [InlineKeyboardButton("📊 البيانات الأساسية وحالة الاشتراكات فقط", callback_data="notes_no")],
            [InlineKeyboardButton("📋 البيانات الأساسية + سجل ملاحظات الأدمن التراكمية", callback_data="notes_yes")]
        ]
        await query.edit_message_text("📁 هل ترغب في تضمين سجل الملاحظات والمتابعات التراكمية في التقرير؟", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
        return EXPORT_INCLUDE_NOTES

async def export_get_uid_or_type(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if target == "exp_single" and "exp_uid" not in context.user_data:
        ...
        keyboard = [
            [InlineKeyboardButton("📊 البيانات الأساسية وحالة الاشتراك فقط", callback_data="notes_no")],
            [InlineKeyboardButton("📋 البيانات الأساسية + سجل ملاحظات الأدمن التراكمية", callback_data="notes_yes")]
        ]
        await update.message.reply_text("📁 هل ترغب في تضمين سجل الملاحظات والمتابعات التراكمية في هذا التقرير؟", reply_markup=InlineKeyboardMarkup(keyboard))
        return EXPORT_INCLUDE_NOTES
...
async def export_generate_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_document(
        ...
        caption="✅ **تم إعداد وتصدير التقرير المطلوب بنجاح! (ملف Excel)**",
        parse_mode="Markdown"
    )
    return ConversationHandler.END
...
async def set_contact_finish(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.commit()
    conn.close()
    await update.message.reply_text("✅ تم تحديث رابط التواصل بنجاح.")
    return ConversationHandler.END
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_owner(user_id):
        keyboard = [
            ...
            [InlineKeyboardButton("📤 تصدير تقارير البيانات (Excel)", callback_data="btn_export_data")],
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "buy":
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📩 تواصل مع الكوتش", url=get_contact_link())]]),
            parse_mode="Markdown"
        )
...
def main():
    ...
    app.job_queue.run_repeating(daily_motivation_job, interval=86400, first=20)
    ...
    app.add_handler(ConversationHandler(
        ...
        states={
            ...
            EXPORT_TYPE: [MessageHandler(filters.TEXT & ~filters.COMMAND, export_get_uid_or_type)],
            EXPORT_INCLUDE_NOTES: [CallbackQueryHandler(export_generate_file, pattern="^notes_")],
        },
        fallbacks=[CommandHandler("cancel", cancel_conv)]
    ))
    ...
    print("🚀 البوت شغال بكامل الإضافات والإصابات والتحديثات بنجاح!")
    app.run_polling()
