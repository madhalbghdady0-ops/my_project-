...
REMOVE_USER_STEP = range(19, 20)  # مرحلة حذف المشترك الجديده

# --- مراحل تسجيل بيانات الـ VIP الجديدة ---
...
async def vip_reg_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    await query.edit_message_text("أهلاً بك يا وحش في برنامج الـ VIP. من فضلك أدخل **عمرك** بالأرقام:", parse_mode="Markdown")
    return VIP_AGE
...
async def vip_finish(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"تم إرسال تفاصيلك للإدارة لمراجعتها وإرسال الجدول الكامل قريباً!",
        parse_mode="Markdown"
    )
    ...
    admin_report = (
        f"🚨 **مشترك VIP جديد سجل بياناته!**\n\n"
        f"👤 اسم المستخدم: @{update.effective_user.username or 'لا يوجد'}\n"
        f"🆔 الآيدي: `{chat_id}`\n"
        f"📌 العمر: {age} سنة\n"
        f"📏 الطول: {height} سم\n"
        f"⚖️ الوزن الحالي: {weight} كجم\n"
        f"🏷️ النوع: {gender}\n"
        f"🎯 الهدف: {goal} (إلى {target_weight} كجم)\n\n"
        f"📋 **التقييم والحسبة المقترحة:**\n"
        f"• السعرات: {macros['calories']} kcal\n"
        f"• بروتين: {macros['protein']}g | كارب: {macros['carbs']}g | دهون: {macros['fats']}g\n"
        f"💡 التوصية: {macros['advice']}"
    )
...
async def service_feedback_finish(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    report = f"💡 **مقترح جديد لتطوير الخدمة والمنصة:**\n\nمن: {user_info}\nالنص: {feedback_text}"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_owner(user_id):
        ...
        keyboard = [
            [InlineKeyboardButton("🚀 تسجيل بيانات اشتراك الـ VIP", callback_data="start_vip_reg")],
            ...
            [InlineKeyboardButton("💡 مقترحات تطوير الخدمة والمنصة", callback_data="service_feedback_btn")],
...
def main():
    ...
    service_feedback_conv = ConversationHandler(
        ...
        states={
            BROADCAST_MSG: [MessageHandler(filters.TEXT & ~filters.COMMAND, service_feedback_finish)],
        },
        fallbacks=[CommandHandler("cancel", cancel_conv)]
    )
    ...
    remove_user_conv = ConversationHandler(
        ...
        states={
            REMOVE_USER_STEP: [MessageHandler(filters.TEXT & ~filters.COMMAND, remove_user_finish)],
        },
        fallbacks=[CommandHandler("cancel", cancel_conv)]
    )
    ...
    print("🚀 البوت شغال بالكامل وبكل الميزات الجديدة!")
    app.run_polling()
