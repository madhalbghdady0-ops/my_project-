...
# مرحلة تعديل الوزن الجديدة للإدارة
...
async def admin_stats_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for idx, (uid, name, end_date_str) in enumerate(all_users, 1):
        ...
        # إضافة أزرار انلاين لكل مشترك للتحكم فيه (تعديل أو حذف مباشر)
        keyboard.append([
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # التعامل مع أزرار الحذف والتعديل الفوري من لوحة الأدمن
...
# دالة لحفظ التعديلات التي يكتبها الأدمن بعد الضغط على زر تعديل المشترك
...
def main():
    ...
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND & filters.User(user_id=DEFAULT_OWNER_ID), handle_admin_edit_text)) # معالجة تعديل الأدمن الفوري
    app.add_handler(CallbackQueryHandler(button_handler))
    app.add_handler(CallbackQueryHandler(admin_stats_handler, pattern="btn_admin_stats"))

    print("🚀 البوت شغال بكامل الإضافات والإصابات والتحديثات وأزرار التعديل بنجاح!")
    app.run_polling()
