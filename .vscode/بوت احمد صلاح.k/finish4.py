...
VIP_AGE, VIP_HEIGHT, VIP_WEIGHT, VIP_GENDER, VIP_GOAL, VIP_TARGET_WEIGHT = range(20, 26)
...
def is_subscribed(user_id):
    """دالة تفحص هل المستخدم لديه اشتراك ساري المفعول حالياً أم لا"""
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
...
# --- محادثة تسجيل بيانات الـ VIP (تلقائية بعد تفعيل الاشتراك) ---
...
async def vip_finish(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    admin_report = (
        f"🚨 **مشترك VIP جديد أكمل بياناته الشخصية!**\n\n"
...
async def sub_date_finish(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # إشعار المشترك بقبول اشتراكه وبدء إدخال بياناته الشخصية تلقائياً
    try:
        await context.bot.send_message(
            chat_id=int(uid), 
            text=f"🎉 **مبروك يا وحش! تم قبول اشتراكك بنجاح.**\nينتهي اشتراكك في تاريخ: `{end_date}`\n\nالآن نبدأ خطوة تسجيل بياناتك الرياضية.\nمن فضلك أدخل **عمرك** بالأرقام:", 
            parse_mode="Markdown"
        )
        # تخزين حالة أن هذا المستخدم بدأ في تسجيل بيانات الـ VIP
        context.user_data["vip_registering_active"] = True
    except:
        pass
    return ConversationHandler.END
...
# --- واجهة البداية واللوحات الذكية (المتكاملة) ---
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_owner(user_id):
        ...
        # لو مش مشترك، يظهر له زرار طلب الاشتراك VIP فقط مع التواصل
        if not subscribed:
            keyboard.append([InlineKeyboardButton("💳 طلب اشتراك VIP", callback_data="buy")])
        else:
            # لو مشترك ساري، تظهر له المنظومة المتكاملة (عرض النظام والأخطاء والمقترحات) وبدون زرار الشراء
        ...
        # زرار التواصل مع الكوتش دايماً موجود
        keyboard.append([InlineKeyboardButton("📩 تواصل مع الكوتش", url=coach_url)])
        
        # الأزرار المخصصة لو الإدارة ضافتها
        for b_key, b_name in custom_btns:
            keyboard.append([InlineKeyboardButton(b_name, callback_data=b_key)])

        status_text = "💎 **أنت مشترك VIP ساري معنا!**" if subscribed else "🚀 أهلاً بك في البوت الرياضي، اختر من الأزرار أدناه:"
        await update.message.reply_text(
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "buy":
        await query.edit_message_text(
            "💳 **طلب اشتراك VIP:**\n\nلتفعيل الاشتراك، يرجى التواصل مع الكوتش مباشرة عبر زر التواصل أدناه لإتمام الدفع، وسيتم تفعيل حسابك فوراً!",
        ...
        if not is_subscribed(user_id):
            await query.edit_message_text("⚠️ عذراً، هذه الميزة مخصصة للمشتركين المشتركين فقط. برجاء تجديد اشتراكك.")
            return
...
def main():
    ...
    # محادثة تسجيل بيانات الـ VIP (تلقائية بالرسائل العادية بعد تفعيل الاشتراك)
    ...
    print("🚀 البوت شغال بالكامل ومنظومة الاشتراكات والبيانات متكاملة!")
    app.run_polling()
