...
# --- أوامر التحكم في الملكية والأدمنز (للتيرمنال) ---
async def set_owner_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.commit()
    conn.close()
    await update.message.reply_text(f"👑 تم نقل الملكية إلى: `{new_owner}`", parse_mode="Markdown")
...
async def add_admin_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.commit()
    conn.close()
    await update.message.reply_text(f"✅ تم إضافة المشرف `{new_admin}` بنجاح.")
...
# --- محادثة إضافة كود الخصم (خطوة بخطوة بالأزرار) ---
...
# --- محادثة النظام اليومي (تفاعلية بالأزرار للأيام) ---
...
async def plan_get_uid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # عرض أيام الأسبوع في أزرار تفاعلية
...
# --- البداية والواجهات الذكية (زبون vs أدمن) ---
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # لو الشخص أدمن أو مالك: تظهر له لوحة الإدارة التفاعلية بالكامل بالأزرار
    if is_admin(user_id):
        keyboard = [
        ...
        await update.message.reply_text(
            f"🛠 **أهلاً بك يا كوتش {user.first_name} في لوحة التحكم التفاعلية:**\nاختر العملية المطلوبة:",
            reply_markup=InlineKeyboardMarkup(keyboard),
            parse_mode="Markdown"
        )
    else:
        # واجهة الزبون العادي (نظيفة وخالية من تفاصيل الإدارة)
        keyboard = [
...
def main():
    ...
    # محادثة إضافة النظام (مع اختيار الأيام بالأزرار)
    ...
    print("🚀 البوت يعمل الآن بالكامل: لوحة تحكم تفاعلية كاملة بالأزرار للأدمن، وواجهة مخصصة للزبائن!")
    app.run_polling()
