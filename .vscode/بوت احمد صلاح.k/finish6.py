...
ADD_VIP_UID, ADD_VIP_DAYS = range(20, 22) # مراحل إضافة مشترك جديد للـ VIP
...
# --- محادثة إضافة مشترك جديد للـ VIP من لوحة التحكم ---
...
async def add_vip_finish(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = sqlite3.connect(DB_FILE)
    # تسجيل المشترك أو تحديثه وتفعيل تاريخ انتهائه
    ...
    # تنبيه المشترك وإرسال رسالة ترحيبية تطلب منه البدء في إدخال بياناته
    try:
        await context.bot.send_message(
            chat_id=int(uid),
...
# --- محادثة تسجيل بيانات الـ VIP (تلقائية بعد القبول) ---
...
def main():
    ...
    # محادثة إضافة مشترك جديد من لوحة التحكم
    ...
    print("🚀 البوت شغال بالكامل مع زرار إضافة المشتركين الجديد!")
    app.run_polling()
